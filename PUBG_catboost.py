...
# model = CatBoostRegressor(random_state=17, learning_rate=0.1, max_depth=5, verbose=2)
...
#     print("training time: ", train_end - train_start, file = file)
# with open('result.txt','w') as file:
#     print(y_pred,file = file)
#     print("mse: ",mse,file=file)


model = CatBoostRegressor(task_type="GPU", learning_rate = 0.1)
adj_params = {'n_estimators': range(100,300,10),
              'max_depth': range(5, 10)  
             }
# model with gpu random search cv
...
y_pred = model.predict(X_test)
mse = mean_squared_error(y_test,y_pred)
with open('result_gpu.txt','w') as file:
    print(y_pred,file = file)
    print("mse: ",mse,file=file)
